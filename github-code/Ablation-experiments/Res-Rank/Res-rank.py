...
resnet_true.classifier = Linear(2048, 5)
model = resnet_true
...
for epoch in range(num_epochs):
    ...
    # save model
    torch.save(model.state_dict(), "Res-rank-last.pt")
    if acc_val == max(val_acc_list):
        torch.save(model.state_dict(), "Res-rank-best.pt")
...
plt.savefig('Res-rank-128-1e-4-数增-1e-4.png')
plt.show()
