...
model = SwinForImageClassification.from_pretrained('../SwinT_weights')
model.load_state_dict(torch.load('../SwinT_weights/pytorch_model.bin'))   # 加载预训练权重
...
for epoch in range(100):
    ...
    # save model
    torch.save(model.state_dict(), "SwinT256-last.pt")
    if acc_val == max(val_acc_list):
        torch.save(model.state_dict(), "SwinT256-best.pt")
...
plt.savefig('SwinT-256-1e-4-数增-1e-4.png')
plt.show()
print("运行完毕")
